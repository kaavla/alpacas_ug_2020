 ...
 * allows you to run your own logic beside the robot.update() command. This enables one to run
...
public class shankAsyncFollowing extends casperAutonomousBase {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Trajectory trajectory1 = robot.trajectoryBuilder(traj0.end())
                .splineTo(new Vector2d(45, -20), Math.toRadians(90))
                .build();

        // Second trajectory
        // Ensure that we call trajectory1.end() as the start for this one
        Trajectory trajectory2 = robot.trajectoryBuilder(trajectory1.end())
        ...
        Trajectory trajectory3 = robot.trajectoryBuilder(newLastPose)
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            switch (currentState) {
                case TRAJECTORY_1:
                    // Check if the robot class isn't busy
                    ...
                    if (!robot.isBusy()) {
                        ...
                        robot.followTrajectoryAsync(trajectory2);
                    }
                    break;
                case TRAJECTORY_2:
                    // Check if the robot class is busy following the trajectory
                    // Move on to the next state, TURN_1, once finished
                    if (!robot.isBusy()) {
                        currentState = State.TURN_1;
                        robot.turnAsync(turnAngle1);
                    }
                    break;
                case TURN_1:
                    // Check if the robot class is busy turning
                    // If not, move onto the next state, TRAJECTORY_3, once finished
                    if (!robot.isBusy()) {
                        currentState = State.TRAJECTORY_3;
                        robot.followTrajectoryAsync(trajectory3);
                    }
                    break;
                case TRAJECTORY_3:
                    // Check if the robot class is busy following the trajectory
